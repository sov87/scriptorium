...
def ensure_registry(reg: Any) -> Dict[str, Any]:
    ...
    reg.setdefault("generated_utc", utc_now())
    reg.setdefault("corpora", [])
...
def main() -> int:
    ...
    ap.add_argument("--harvest", required=True, help="Harvest JSON from ingest_perseus_all.py")
    ap.add_argument("--registry", default="docs/corpora.private.json")
    ...
    by_id: Dict[str, Dict[str, Any]] = {c.get("corpus_id"): c for c in corpora if isinstance(c, dict) and c.get("corpus_id")}
    ...
    if missing and args.fail_missing:
        raise SystemExit("[FATAL] missing JSONL:\n" + "\n".join(missing[:100]))
    ...
    if missing:
        print(f"[WARN] missing jsonl skipped={len(missing)}")
    return 0
